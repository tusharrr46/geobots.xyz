...
// Types imports
import { MarkersContextType, MarkerType } from 'utils/types/markers';
import { ContextProviderProps } from 'utils/types/context';
import { DataProvider } from 'utils/types/map';
...
/**
 * Manages all marker state and interaction: create, update, reject
...
export const MarkersProvider = ({children}: ContextProviderProps) => {
	...
	const [ currentProvider, setCurrentProvider ] = useState<DataProvider | null>(null);

	const startAddingMarker = useCallback((src: string, provider: DataProvider) => {
	...
	const createMarker = useCallback((id: number, center: { lng: number; lat: number }): MarkerType => ({
		...
		layerType: currentProvider?.layerType,
		boundary: null,
		data: null,
		...(currentProvider ? Object.fromEntries(
			Object.entries(currentProvider).filter(([key]) => !['layer', 'layerType'].includes(key))
		) : {}),
